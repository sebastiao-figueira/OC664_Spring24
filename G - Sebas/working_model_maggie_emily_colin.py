    ...
    for i in range(0, len(psimax)):
        ...
        while abs(new_calc - last_calc) >= epsilon:  # testing convergence
            ...
            if np.divide(a_hat[i],
                         ksw[i]) > 1.587:  # Swart Equation, which goes into the time-averaged absolute Shields stress
...
# Colin/Emily output:
shields_aa, f_wc, f_wt, ksdelta, ksw, fdelta, f_w = combined_wavefric_ripples(T_cu, T_c, T_tu, T_t, a_hat, u_hat,
                                                                              u_delta, delta, d50, d90, s, g)
# Maggie output:
fwdelt_c, fwdelt_t, TwRe, theta_cmag, theta_tmag, theta_cx, theta_tx = currentfric(u_delta, u_hat, f_wc, f_wt, rho, f_w,
                                                                                   c_w, u_crx, u_trx, s, g, d50, fdelta)

# Carly output:
dstar = dimensionless_grainsize(d50)
shields_cr = critical_shields(dstar)
omega_c = sandload_crest(theta_cx, shields_cr)
omega_t = sandload_trough(theta_tx, shields_cr)

# Carson output:
sheetflow_thickness_c = sfl_thickness(theta_cmag, d50)
sheetflow_thickness_t = sfl_thickness(theta_tmag, d50)
