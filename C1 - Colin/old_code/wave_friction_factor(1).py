...
# This function calculates the wave related friction factor and the 'time-averaged absolute Shields stress'.
...
def wavefric(T_cu, T_c, T_tu, T_t, a_hat, u_hat, u_deltamag, k_sw, f_delta, d50, s, g):
    c1 = 2.6 # noted above
    
    # Swart Equation, which goes into the time-averaged absolute Shields stress below
    if a_hat/k_sw > 1.587:
        f_w = 0.00251*np.exp(5.21*((a_hat/k_sw)**(-0.19))) # equation A.4; the Swart Equation. In the absence of acceleration skewness f_wc/f_wt below reduce to this
    else:
        f_w = 0.3
